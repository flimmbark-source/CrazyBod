...
- **Discomfort:** click the highlighted left/right adjustment, or use `A` / `D` or the left/right arrows.
- **Anxiety:** click the active pulse targets.
- **Brain Fog:** click the window, then use the arrow keys or `WASD`. On-screen arrows remain available.
- **Fatigue:** press and hold the button, or hold `Space`. Some variants require releasing between pushes.
