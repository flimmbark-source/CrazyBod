...
// The scored day starts at the front door: everything before it now happens in
// the untimed Morning (see MorningHouse.jsx), which parks the camera at
...
// the room, with the front door closed. The Morning drives the scene with this
// fixed value rather than a running clock.
export const MORNING_ELAPSED = -6

// The Morning is authored as its own short keyframe so the bedroom still reads
// as a room you are standing in rather than a frozen frame of the walk.
const MORNING_POSE = {
  position: [0.55, 1.65, 3.1],
  look: [0.4, 1.44, -4.6],
  walk: 0,
  fov: 70,
}
...
function samplePlayerPath(elapsed) {
  // Anything before the day proper is the Morning pose.
  if (elapsed < 0) return MORNING_POSE
...
function Bedroom({ elapsed }) {
  ...
  // day opens on it already swinging.
  const apartmentDoorProgress = clamp01(
    (elapsed - APARTMENT_DOOR_OPENS_AT) / APARTMENT_DOOR_SWING_SECONDS,
  )
